...
export default function DryBakeForm({ ovens = [], chambers = [],  packageLots = [] }) {
...
useEffect(() => {
    const handleClickOutside = (event) => {
        if (chamberRef.current && !chamberRef.current.contains(event.target)) {
    ...
    const normalizedChambers = (Array.isArray(chambers) ? chambers : []).flatMap((item) => {
...
const chamberMap = normalizedChambers.reduce((acc, item) => {
    ...
    */
    const saveAll = () => {
         setProcessing(true);
        router.post(route("dry-bake.bulk-store"), {
            data: rows,
        }, {
            onSuccess: () => {
            ...
            <div className="flex flex-col">
                ...
                <select
                    ...
                    onChange={(e) => {
                        ...
                        setAvailableChambers([...new Set(found.flat())]);
...
onChange={(e) => {
    ...
    const found = packageLots.find((item) =>
        item.Lot_Id.toUpperCase().includes(value)
    );

    if (found) {
        setData("partname", found.Part_Name || "");
        setData("package", found.Package_Name || "");
        setData("qty", found.Qty || "");

        const bake = found.Bake_Time_Temp || "";

        // split OR cases
        const options = bake.split(/OR/i).map((s) => s.trim());

const parsed = options
    .map((opt) => {
        if (!opt) return null;

        const cleaned = opt.toUpperCase().replace(/AT/gi, "@");

        // TIME (handles spaces automatically)
        const timeMatch = cleaned.match(/(\d+)\s*HRS?/i);

        // TEMP (handles: 125'C, 125' C, 125C, +135'C)
        const tempMatch = cleaned.match(/(\+?\d+)\s*'?\s*C/i);

        // fallback kung baliktad format
        if (!timeMatch || !tempMatch) {
            const reverse = cleaned.match(/(\+?\d+)\s*'?\s*C.*?(\d+)\s*HRS?/i);
        ...
        if (!timeMatch || !tempMatch) return null;
    ...
    .filter(Boolean);
        ...
        if (parsed.length === 1) {
            ...
            setData("temperature", parsed[0].temp);

            setTimeOptions([]);
            setTempOptions([]);
        } else if (parsed.length > 1) {
            // MULTIPLE MODE (SELECT MODE)
            setTimeOptions([...new Set(parsed.map((p) => p.time))]);
            setTempOptions([...new Set(parsed.map((p) => p.temp))]);
    ...
    {showChamberDropdown && (
        <div className="absolute top-full mt-1 w-full bg-white border rounded shadow max-h-40 overflow-y-auto z-10">
            {(chamberMap[data.oven] || [])
                .filter((ch) =>
                    ch.toLowerCase().includes(data.chamber.toLowerCase())
                )
                .map((ch, i) => (
                    <div
                        key={i} 
            ...
            {/* pag walang result */}
            {(chamberMap[data.oven] || []).filter((ch) =>
                ch.toLowerCase().includes(data.chamber.toLowerCase())
            ).length === 0 && (
            ...
            {/*Input Type*/}
             <div className="flex flex-col">
                ...
                <select name="input_type" id="" className="border w-full rounded p-2 text-gray-600" onChange={(e) => setData("input_type", e.target.value)}>
